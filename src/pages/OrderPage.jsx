...
export default function OrderPage({
      ...
      if (Array.isArray(updatedOrders)) {
        if (!currentUser) {
          ...
          const userOrders = updatedOrders.filter((o) => {
            return (
              (userId && (o.user_id === userId || o.customer_id === userId)) ||
              (customerId && o.customer_id === customerId) ||
              (name && o.customer_name && o.customer_name.toLowerCase().includes(name))
            );
          });
          setOrdersList(userOrders.length > 0 ? userOrders : updatedOrders);
        }
      }
    });
    return () => unsubscribe();
  ...
  const countsByStatus = useMemo(() => {
    const counts = { All: ordersList.length, 'Pending Approval': 0, Processing: 0, Shipped: 0, Delivered: 0, Cancelled: 0 };
  ...
  const handleBuyAgain = (order) => {
    ...
    order.items.forEach((item) => {
      ...
      const matchingProduct =
        productsList.find((p) => p.id === item.product_id || p.name.toLowerCase() === item.name.toLowerCase()) || {
          ...
          {ordersList.length === 0 ? (
            <View style={styles.emptyStateWrap}>
              ...
              <Text style={styles.emptySubtitle}>
                You have not placed any orders yet. Discover our latest motorcycle spare parts, performance upgrades, and racing components to place your first order.
              </Text>
              <TouchableOpacity
                  ...
                  <View key={order.order_id || order.id || `order-${orderIdx}`} style={styles.orderCard}>
                      ...
                      <View style={styles.orderHeaderLeft}>
                        ...
                        <View>
                          ...
                          <Text style={styles.orderDateText}>
                            Placed on {order.order_date || 'Aug 20, 2026'} • {order.items_count || order.items?.length || 1} {order.items_count === 1 ? 'item' : 'items'}
                          </Text>
                        </View>
                      </View>
                    ...
                    <View style={styles.orderItemsWrap}>
                      {order.items && order.items.length > 0 ? (
                        order.items.map((item, idx) => (
                          <View key={idx} style={styles.orderItemRow}>
                            <Image
                              source={{ uri: item.image || 'https://images.unsplash.com/photo-1530549387789-4c1017266635?auto=format&fit=crop&w=800&q=80' }}
                            ...
                            <View style={styles.orderItemPriceCol}>
                              <Text style={styles.orderItemPrice}>₱{(Number(item.price || 0) * (item.quantity || 1)).toFixed(2)}
                              </Text>
                            </View>
                          </View>
                        ))
                      ) : (
                        <Text style={{ fontSize: 13.5, color: '#475569' }}>
                          {order.items_summary}
                        </Text>
                      )}
                      ...
                      >
                        ...
                        <Text style={{ fontSize: 12, color: '#475569', flex: 1 }} numberOfLines={1}>
                          Shipping to: <Text style={{ fontWeight: '700' }}>{order.customer_name || 'Alex Rider'}</Text> ({order.customer_address || '742 Evergreen Terrace, Springfield, OR'})
                        </Text>
                      </View>
                    </View>
                    ...
                    <View style={styles.orderCardFooter}>
                      <View style={styles.orderMetaCol}>
                        ...
                        <Text style={styles.orderTotalValue}>₱{Number(order.grand_total || order.total_amount || 0).toFixed(2)}
                        </Text>
                      ...
                      <View style={styles.orderActionsRow}>
                        ...
                        >
                          ...
                          <Text style={styles.trackShipmentBtnText}>
                            Track on Live Map 🗺️
                          </Text>
                        </TouchableOpacity>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                <View style={styles.invoicePaper}>
                  ...
                  <View style={styles.invoiceHeader}>
                    <View>
                      <Text style={{ fontSize: 20, fontWeight: '900', color: '#0F172A' }}>
                        Moto<Text style={{ color: '#0C6258' }}>Track</Text> Pro Gear
                      </Text>
                  ...
                  <View style={{ marginBottom: 16 }}>
                    <Text style={{ fontSize: 11.5, fontWeight: '800', color: '#64748B', textTransform: 'uppercase' }}>
                      Billed & Shipped To:
                    </Text>
                    ...
                    <Text style={{ fontSize: 12.5, color: '#475569' }}>
                      {selectedOrderForInvoice.customer_address || '742 Evergreen Terrace, Springfield, OR 97477'}
                    </Text>
                  ...
                  {/* Itemized list */}
                  <Text style={{ fontSize: 11.5, fontWeight: '800', color: '#64748B', textTransform: 'uppercase', marginBottom: 6 }}>
                    Purchased Items:
                  </Text>
                    ...
                    selectedOrderForInvoice.items.map((item, idx) => (
                      <View key={idx} style={styles.invoiceItemRow}>
                        <Text style={styles.invoiceItemName}>
                          {item.name} <Text style={{ color: '#64748B', fontWeight: '600' }}>(x{item.quantity || 1})</Text>
                        </Text>
                        <Text style={styles.invoiceItemPrice}>₱{(Number(item.price || 0) * (item.quantity || 1)).toFixed(2)}
                        </Text>
                      </View>
                    ))
                  ) : (
                  ...
                  {/* Pricing breakdown */}
                  <View style={{ marginTop: 14, paddingTop: 10, borderTopWidth: 1, borderTopColor: '#E2E8F0', gap: 6 }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      <Text style={{ fontSize: 12.5, color: '#64748B' }}>Subtotal</Text>
                      <Text style={{ fontSize: 12.5, fontWeight: '700', color: '#0F172A' }}>₱{Number(selectedOrderForInvoice.total_amount || selectedOrderForInvoice.grand_total || 0).toFixed(2)}
                      </Text>
                    </View>
                    ...
                    <View style={styles.invoiceTotalRow}>
                      ...
                      <Text style={styles.invoiceTotalVal}>₱{Number(selectedOrderForInvoice.grand_total || selectedOrderForInvoice.total_amount || 0).toFixed(2)}
                      </Text>
                    </View>
                  </View>
                </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { maxWidth: 420, alignItems: 'center' }]}>
            ...
            <Text style={{ fontSize: 13.5, color: '#64748B', textAlign: 'center', lineHeight: 20, marginBottom: 20 }}>
              Are you sure you want to cancel this order? Once cancelled, the items will not be dispatched and a full refund will be credited back to your payment method.
            </Text>
            ...
            <View style={{ flexDirection: 'row', gap: 10, width: '100%' }}>
              ...
              <TouchableOpacity
                style={{ flex: 1, backgroundColor: '#DC2626', paddingVertical: 11, borderRadius: 12, alignItems: 'center' }}
